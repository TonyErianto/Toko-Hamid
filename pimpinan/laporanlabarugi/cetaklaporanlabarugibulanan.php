...
<head>
    ...
    <title>Laporan Laba Rugi Bulan <?php echo  date('M-Y', strtotime($_GET["bulan"])) ?>  - Toko Hamid Furniture Padang</title>
</head>
<style>
    hr{
...
<body> 
                    ...
                    <p style="text-align:center">Laporan Laba Rugi Bulanan</p>
                    <p style="text-align:center">Bulan : <?php echo date('M-Y', strtotime($_GET["bulan"]))?></p>
                    <hr>
                     <?php
                        $bulanini=$_GET['bulan'];
                        $totalmodaljual="SELECT SUM(total_harga_barang_modal) AS totalmodal, SUM(total_harga_barang_jual) AS totaljual FROM tb_penjualan  WHERE tanggal_transaksi LIKE '%$bulanini%' ORDER BY tanggal_transaksi ASC"; 
                        $totalgaji  = "SELECT SUM(harga_beban) AS hargagaji FROM tb_data_beban WHERE nama_beban='Gaji Karyawan' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totallistrik = "SELECT SUM(harga_beban) AS hargalistrik FROM tb_data_beban WHERE nama_beban='Listrik Ruko' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totalinternet = "SELECT SUM(harga_beban) AS hargainternet FROM tb_data_beban WHERE nama_beban='Internet Ruko' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totalair = "SELECT SUM(harga_beban) AS hargaair FROM tb_data_beban WHERE nama_beban='Air PDAM' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totalperlengkapan = "SELECT SUM(harga_beban) AS hargaperlengkapan FROM tb_data_beban WHERE nama_beban='Perlengkapan' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totalperbaikan = "SELECT SUM(harga_beban) AS hargaperbaikan FROM tb_data_beban WHERE nama_beban='Perbaikan' AND tanggal_transaksi LIKE '%$bulanini%' ";
                        $totalbeban= "SELECT SUM(harga_beban) AS hargabeban FROM tb_data_beban WHERE tanggal_transaksi LIKE '%$bulanini%' ";
                    ?>
                    <table>
                        <?php 
                        ...
                        <tr>
                            <th style="text-align:left;font-weight:normal;padding:10px;font-size:13px">Total Laba Kotor : <?php $totalkotor=$totaljual-$totalmodal echo "Rp. ". number_format($totalkotor, 2, ".", ",");?></th>
                        </tr>
                        <?php
                        }
                        ?>
                        ...
                        ?>
                        <tr>
                           <th style="text-align:left;font-weight:normal;padding:10px;font-size:13px">Total Laba Bersih Operasional <?php $totallababersih=$totalkotor-$hargabeban echo "Rp. ". number_format($totallababersih, 2, ".", ",");?>  ?></th>
                        </tr>
                        <tr>
                           <th style="text-align:left;font-weight:normal;padding:10px;font-size:13px">Total Laba/Rugi Bersih :<?php $totallababersih=$totalkotor-$hargabeban echo "Rp. ". number_format($totallababersih, 2, ".", ",");?>  ?></th>
                        </tr>
                    </table>
...
<?php
$filename="Laporan Laba Rugi - Bulan ".date('d-m-Y', strtotime($_GET["bulanini"])).".pdf";
